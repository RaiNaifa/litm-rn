export class TokenTooltip {
	...
	_matchesHighlightRelease(event) {
		...
		return bindings.some((b) => {
			...
			if (mods.includes("Alt") && (code === "AltLeft" || code === "AltRight"))
				return true;
			if (
	...
	_showAltTooltips() {
		...
		const tokens = this._getVisibleTokensOnCanvas();
	...
	_isTokenVisible(token) {
		...
		return (
			(config.actors || []).some((a) => a.ref === actorUuid) ||
			(config.tokenTagVisibility || []).includes(actorUuid)
		);
	}
	...
	_getVisibleTokensOnCanvas() {
		...
		const visibleRefs = new Set([
			...(config.actors || []).map((a) => a.ref),
			...(config.tokenTagVisibility || []),
		]);
		if (!visibleRefs.size) return [];
		return canvas.tokens.placeables.filter(
			(t) => t.actor?.uuid && visibleRefs.has(t.actor.uuid),
		);
	}
	...
	_collectEffects(actor) {
		...
		return actor.effects
			.filter((e) => {
				...
				const t = f?.type;
				if (t !== "tag" && t !== "status" && t !== "might") return false;
			...
			.map((e) => {
				...
				if (rawType === "might") {
					const val = f.value ?? 3;
					const tier =
						val >= 6 ? "greatness" : val >= 3 ? "adventure" : "origin";
					return { name: e.name, type: "might", tier, value: val, isPrivate };
				}

				if (rawType === "status") {
					const lastIdx = values.findLastIndex((v) => !!v);
					const statusLevel = lastIdx >= 0 ? lastIdx + 1 : null;
					return {
						name: e.name,
						type: "status",
						value: statusLevel,
				...
				return { name: e.name, type: "tag", isPrivate };
			})
			.sort((a, b) => {
				const order = { status: 0, tag: 1, might: 2 };
	...
	_renderMarks(effects) {
		return effects
			.map((e) => {
				...
				if (e.type === "status") {
					const val = e.value
						? `<span class="litm--hover-status-val">${e.value}</span>`
						: "";
					return `<mark class="litm--hover-status${privateCls}">${e.name}${val}</mark>`;
				}
				if (e.type === "might") {
					return `<mark class="litm--hover-might litm--hover-might-${e.tier}${privateCls}">${e.name}</mark>`;
				}
